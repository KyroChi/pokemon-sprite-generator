...
def evaluate(config, model, epoch, x0_eval):
    steps = config.num_inference_timesteps

    sample = (sample_iadb(model, x0_eval, nb_step=steps) * 0.5) + 0.5
...
class BaselineImageIADB(Experiment):
    def __init__(self, **kwargs):
        ...
        self.model.load_state_dict(
            load_file('/home/kyle/projects/pokemon_diffusion/_testing_runs/baseline_img_iadb/_64_64_pretrained_slight_overfit/models/model_32.pt/model.safetensors')
        )
    ...
    def train_evaluation(self, epoch, *args, **kwargs):
        if (epoch + 1) % self.config.save_image_epochs == 0 or epoch == self.config.num_epochs - 1:
            evaluate(self.config, self.model, epoch + 1, self.x0_eval)
...
if __name__ == "__main__":
    training_metadata = {
        'experiment_name': 'baseline_img_iadb',
